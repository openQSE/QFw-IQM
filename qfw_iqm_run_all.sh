...
(cd "${QFW_PATH}" && qfw_srun.sh "${repo_dir}/scripts/iqm_env_check.py" --json)
(cd "${QFW_PATH}" && qfw_srun.sh "${repo_dir}/scripts/iqm_discover.py" --json)
(cd "${QFW_PATH}" && qfw_srun.sh "${repo_dir}/scripts/iqm_submit_smoke.py" --json)
